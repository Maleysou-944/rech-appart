...
Champs extraits par annonce : `url`, `prix`, `surface`, `ville`, `departement`
...
**fetch_html() :** `requests` direct avec User-Agent (pas de ScraperAPI)

**parse() :** sélecteurs CSS à confirmer à partir d'une fixture HTML lors de l'implémentation
